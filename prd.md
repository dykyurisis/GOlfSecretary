...
### 13.1 자동 예약 / 스나이핑 (요청됨 — 2026-06-30) — 차기 우선 후보
- **요구**: 인기 티타임은 경쟁이 심함. 보통 **약 2주 전 오전 6:30경(현지)** 에 해당 날짜 티타임이 열림. 열리는 즉시 원하는 시간대를 자동으로 잡아주길 원함.
- **개념**: 사전에 "워치"(클럽·날짜·원하는 시간대·동반자)를 등록 → 스케줄러가 **오픈 순간**에 사전 로그인된(세션 워밍업) 워커를 깨워 즉시 조회·예약, 1순위 마감 시 차순위로 폴백 → 결과 알림.
- **필요 요소**: 워치/스케줄 데이터 모델, **정확한 오픈 시각 기준의 스케줄러**, 세션 사전 워밍업, 오픈 직후 빠른 폴-앤-그랩 + 폴백, 결과 알림 채널(이메일 또는 2단계 카카오 알림).
- **리스크**: ① 오픈 직후 빠른 반복 동작 → **봇 탐지(Cloudflare) 위험 상승**(경쟁상 속도 필요와 상충). ② **정확한 오픈 타임스탬프** 의존 → **M0의 R3(예약 오픈 날짜+시각) 정밀 확인이 선행 조건**. ③ ToS상 자동 예약 민감도 높음.
- **상태**: 본 기능을 위해 M0에서 오픈 규칙(정확한 날짜 오프셋·시각·초 단위)을 반드시 측정. 정식 스펙은 별도 작성 예정.
